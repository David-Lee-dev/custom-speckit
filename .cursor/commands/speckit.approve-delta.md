...
Merge the approved delta specification changes into `specs/spec.md`, creating a backup and maintaining change history.

This command is WRITE-ENABLED and will modify `specs/spec.md` - proceed with caution.
...
**Prerequisites Check**:
- If HAS_EXISTING_SPEC is false: ERROR "No existing spec found. Cannot merge delta without specs/spec.md."
...
- Ask: "You are about to merge {N} additions, {M} modifications, and {K} deletions into specs/spec.md. This will modify the main specification. Continue? (yes/no)"
...
**CRITICAL**: Always create backup before modifying specs/spec.md

1. Create backup directory if not exists: `mkdir -p {REPO_ROOT}/specs/.backups/`
2. Generate backup filename: `spec_backup_{YYYYMMDD_HHMMSS}_{BRANCH}.md`
3. Copy current specs/spec.md to backup:
   ```bash
   cp specs/spec.md specs/.backups/spec_backup_{timestamp}_{branch}.md
   ```
...
- Apply changes to specs/spec.md in correct order
...
**Error Handling**:
- If SUCCESS is false: 
  - Restore from backup: `cp specs/.backups/spec_backup_{timestamp}_{branch}.md specs/spec.md`
...
1. Create/append to `specs/CHANGELOG.md`:
...
**Delta Location**: `.deltas/{branch}/` (archived)  
**Backup**: `specs/.backups/spec_backup_{timestamp}_{branch}.md`
...
2. If git is available and user allows:
   - Optionally stage changes: `git add specs/spec.md specs/CHANGELOG.md`
...
2. Move delta directory: `mv .deltas/{branch} specs/.deltas-archive/{timestamp}_{branch}/`
3. Add archive note to CHANGELOG.md

**Option B: Delete** (if user prefers):
1. Remove delta directory: `rm -rf .deltas/{branch}/`
...
**A. Sanity checks**:
- specs/spec.md exists and is not empty
...
- `specs/spec.md` - Main specification updated
- `specs/CHANGELOG.md` - Change history recorded
- `specs/.backups/spec_backup_{timestamp}_{branch}.md` - Backup created
...
1. Review the updated `specs/spec.md` to verify changes
...
1. Restore from backup:
   ```bash
   cp specs/.backups/spec_backup_{timestamp}_{branch}.md specs/spec.md
   ```

2. Restore delta (if archived):
   ```bash
   mv specs/.deltas-archive/{timestamp}_{branch} .deltas/{branch}
   ```
...
1. **Always backup**: Never modify specs/spec.md without backup
...
**For Additions**:
- Find appropriate insertion point in specs/spec.md
...
1. Stop immediately
2. Restore specs/spec.md from backup
